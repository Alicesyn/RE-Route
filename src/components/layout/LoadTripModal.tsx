...
export const LoadTripModal: React.FC<LoadTripModalProps> = ({ isOpen, onClose }) => {
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-surface-900/40 backdrop-blur-sm">
        ...
        >
          <div className="flex items-center justify-between p-5 border-b border-surface-100 bg-surface-50/50">
            <div>
              <h2 className="text-xl font-bold text-surface-900">My Saved Trips</h2>
              <p className="text-sm text-surface-500 mt-1">Select a trip to load its full itinerary</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-4 overflow-y-auto custom-scrollbar flex-1 bg-surface-50">
            {savedTrips.length === 0 ? (
              <div className="text-center py-12 px-4 bg-white rounded-xl border border-dashed border-surface-200">
                <p className="text-surface-500 font-medium">You haven't saved any trips yet.</p>
                <p className="text-sm text-surface-400 mt-1">Click "Save" in the header to save your current itinerary.</p>
              </div>
            ) : (
                ...
                {savedTrips.slice().reverse().map((trip) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <h3 className="font-bold text-surface-900 group-hover:text-primary-700 transition-colors text-lg">{trip.title}</h3>
