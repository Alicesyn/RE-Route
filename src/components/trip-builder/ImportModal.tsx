...
import { X, FileText, Loader2, CheckCircle2, AlertCircle, MapPin, Check, Square, CheckSquare } from 'lucide-react';
...
export const ImportModal: React.FC<ImportModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSearchAll = async () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const existingPlace = places.find(p => p.name.toLowerCase() === line.toLowerCase());
      ...
      try {
        ...
        if (appMode === 'real') {
          ...
          // Mock search logic
          mapsResults = MOCK_PLACES
            .filter(p => p.name.toLowerCase().includes(line.toLowerCase()))
            .map(p => ({
              id: p.id,
              name: p.name,
              address: p.address,
  ...
  const toggleAll = () => {
    ...
    setResults(prev => prev.map(r => r.match ? { ...r, selected: !allSelected } : r));
  };
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
              {modalState === 'input' && (
                <div className="space-y-4">
                  <p className="text-sm text-surface-600 dark:text-surface-300">
                    Paste a list of places (one per line). We'll find the best matching locations on Google Maps for you.
                  </p>
                  <textarea
                    value={text}
                  ...
                  <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 py-3 px-4 rounded-xl border border-surface-200 dark:border-surface-700 text-surface-700 dark:text-surface-300 font-semibold hover:bg-surface-50 dark:hover:bg-surface-700 transition-colors">
                      Cancel
                    </button>
                    <button
              ...
              {modalState === 'searching' && (
                <div className="py-12 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-sm text-surface-500 dark:text-surface-400 mt-1 truncate max-w-xs mx-auto">
                    Searching for: <span className="font-medium text-primary-600 dark:text-primary-400">"{progress.currentName}"</span>
                  </p>
              ...
              {modalState === 'review' && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between pb-2 border-b border-surface-50 dark:border-surface-700">
                    ...
                    >
                      {results.every(r => !r.match || r.selected) ? 'Deselect All' : 'Select All'}
                    </button>
                  </div>
                    ...
                    {results.map((res, i) => (
                      <div 
                        ...
                        className={`flex items-start gap-4 p-3 rounded-xl border transition-all ${
                          res.match 
                            ? (res.selected ? 'bg-primary-50/50 dark:bg-primary-900/10 border-primary-200 dark:border-primary-800' : 'bg-white dark:bg-surface-800 border-surface-100 dark:border-surface-700 opacity-60')
                      ...
                      >
                        ...
                        >
                          {res.selected ? <CheckSquare className="w-5 h-5" /> : <Square className="w-5 h-5" />}
                        </button>

                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[11px] font-bold text-surface-400 uppercase tracking-tighter shrink-0">Input:</span>
                            <span className="text-sm font-semibold text-surface-900 dark:text-white truncate">"{res.query}"</span>
                          </div>
                          ...
                          {res.match ? (
                            ...
                            <div className="flex items-center gap-1.5 text-red-600 dark:text-red-400">
                              ...
                              <span className="text-xs font-medium">{res.error}</span>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {modalState === 'success' && (
                <div className="py-8 flex flex-col items-center justify-center text-center">
                  ...
                  <h3 className="text-xl font-bold text-surface-900 dark:text-white mb-2">Import Complete!</h3>
                  <p className="text-sm text-surface-500 dark:text-surface-400 mb-6">
                    Successfully added <span className="font-bold text-emerald-600">{results.filter(r => r.selected).length}</span> places.
                  </p>
                  ...
                  {results.some(r => !r.match) && (
                    <div className="w-full bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-xl p-4 mb-6 text-left">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button 
                          onClick={() => {
                            const failed = results.filter(r => !r.match).map(r => r.query).join('\n');
                      ...
                      <ul className="text-xs text-red-500 space-y-1 max-h-32 overflow-y-auto custom-scrollbar">
                        {results.filter(r => !r.match).map((res, i) => (
